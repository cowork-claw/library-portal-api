...
def benchmark(year=2022, semester=5, iterations=1000):
    ...
    curr_urls = sorted(p["url"] for p in res_curr if "url" in p)
    opt_urls = sorted(p["url"] for p in res_opt if "url" in p)
...
if __name__ == "__main__":
    ...
    if paper_index.unique_years:
        ...
        papers = paper_index.get_by_urls(urls)
        semesters = list({p.get("semester") for p in papers if p.get("semester")})
