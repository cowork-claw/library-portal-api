    ...
    for i, paper in enumerate(papers, 1):
        try:
            ...
            if result.should_auto_write and result.target_file:
                ...
                if dry_run:
                    logger.info(f"DRY RUN: Would write {course_code} to {result.target_file}")
                ...
                if dry_run:
                    logger.info(f"DRY RUN: Would stage {course_code} (conf: {result.confidence:.2f})")
                else:
...
def main():
    ...
    parser.add_argument(
        "input",
        type=Path,
        help="Input JSON file containing scraped papers"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
