...
- **Data Loading Optimization:** The `DataLoader` class uses a `set` (`seen_urls`) for O(1) membership lookups during paper deduplication across files. This eliminates unnecessary dictionary memory allocations and value tracking, making the loading process faster and more efficient at startup.
