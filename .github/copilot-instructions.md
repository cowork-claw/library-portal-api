...
- **Search Optimization:** The fuzzy search logic in `app_v2/services/search.py` uses `rapidfuzz` (replacing `thefuzz`) which provides a ~3x additional speedup. Global search results are cached using `@lru_cache` in `PaperIndex` for instant repeated queries.
